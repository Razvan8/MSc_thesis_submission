...
{ xty<-sum(X*y)
  ...
  c<-lambda*w
  if (scaled ==TRUE)
  {c<-c*length(X)*2}###lasso loss scaled is loss/(2*n)
  #cat(" xtx: ", xtx, " xty: ", xty, " c: ", c )
  #cat(" (xty-c) / xtx:", (xty-c)/xtx, xty/xtx -c/xtx)
  
  result<-Soft_thresholding(xty, c/2)/xtx #should substract just c*lmd 
  return(result)
}
...
poly_lasso_min<-function(coefs, lambda, old_x=0){ #finds the min for a< a=0, a>0 gets the min out of all
  ...
  #cat("xmin opt, val min optimize:", x_min_optimize,' ', val_min_optimize )
  #cat("xmin poly, val min poly:", x_min,' ', poly_min_val )
...
Q_normal<-function(X,y, beta, gamma_vec, delta_vec, lambda_beta, lambda_gamma, lambda_delta, w_beta, w_gamma, w_delta,l1,l2,l3,
                   already_multiplied=TRUE, scaled=TRUE)
...
{beta_2way <- get_beta_vec_2way(beta = beta_hat, l1=l1, l2=l2, l3=l3, gamma=gamma_hat, only_beta = FALSE) ###This is with delta
 ...
 lambda_delta<-lambda_delta *1#lasso in library minimizes 1/(2n)*loss+lambda already
...
update_beta <- function(X, y, beta_hat, gamma_hat, delta_hat, lambda_beta, l1, l2, l3, w=1)
{
  ...
  for (i in range1) {
    ...
    beta_hat[i]<-poly_lasso_min(coefs = coefs, lambda = lambda_beta*2*length(y), old_x = beta_hat_old ) #TAKE CARE!!!!!!! lambda_beta *2n because we used scales
  ...
  for (j in range2) {
    ...
    beta_hat[j]<-poly_lasso_min(coefs = coefs, lambda = lambda_beta*2*length(y), old_x = beta_hat_old) #beta updated
    ...
    for (k in range3) {
      ...
      beta_hat[k]<-poly_lasso_min(coefs = coefs, lambda = lambda_beta*2*length(y), old_x = beta_hat_old) #beta updated
      #update beta_23way
      beta_2way <- get_beta_vec_2way(beta = beta_hat, l1 = l1, l2 = l2, l3 = l3, gamma = gamma_hat, only_beta = FALSE) ###This is with delta
      beta_3way <- get_beta_vec_3way(beta_2way = beta_2way, l1 = l1, l2 = l2, l3 = l3, delta = delta_hat, only_beta = FALSE) #This is with gamma WITH delta
  ...
  
{
  ...
  cross_validation <- function(X, y, lambda_values_main, lambda_values_2way, lambda_delta, intercept, split_percentage = 0.5, k = 5) {
    best_lambda1 <- NULL
    best_lambda2 <- NULL
    best_R2score <- -Inf
    
    # Initialize a matrix to store the average R2 scores for each lambda combination
    R2_scores <- matrix(0, nrow = length(lambda_values_main), ncol = length(lambda_values_2way))
    
    # Perform k train-test splits
    for (i in 1:k) {
      # Split the data
      ...
      for (j in 1:length(lambda_values_main)) {
        ...
        for (l in 1:length(lambda_values_2way)) {
          ...
          fitted <- fit(X = X_train, y = y_train, lambda_beta = lambda1, lambda_gamma = lambda2, lambda_delta = lambda_delta, w_beta = 1, w_gamma = 1, w_delta = 1, tol = 1e-2, max_iter = 20, compute_Q = Q_normal)
          
          # Compute the R2 score
          R2 <- R2_score(self = fitted, X_new = X_test, y_true = y_test)
          
          # Accumulate the R2 score for averaging later
          R2_scores[j, l] <- R2_scores[j, l] + R2
        }
      }
    }
